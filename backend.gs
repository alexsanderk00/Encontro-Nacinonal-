...
/* Prazo final das inscrições: 24/10/2026 23:59:59 no horário de Brasília.
...
const PRAZO_INSCRICAO_MS = new Date('2026-10-25T02:59:59Z').getTime();
...
function validatePayload(data, tabela) {
  ...
  if (data.tipo !== 'produto' && Date.now() > PRAZO_INSCRICAO_MS)
    return { ok: false, message: 'As inscrições foram encerradas em 24 de outubro de 2026.' };
